...
### 7. [unified_analysis.ipynb](unified_analysis.ipynb) ⭐ **신규**
**통합 분석 파이프라인 (All-in-One)**

**모든 경로 타입을 자동으로 처리하는 통합 노트북:**
- ✅ 단일 경로 & 연속 경로 통합 처리
- ✅ Toyo & PNE 자동 감지
- ✅ 동일한 출력 파이프라인 (수명, 프로파일, 시각화, DB)
- ✅ 4가지 시나리오 완벽 지원
...
**출력 결과:**
- ⚡ 수명 분석 (Cycle Analysis)
- 📊 프로파일 분석 (Profile Analysis)
- 📈 시각화 (4개 차트: 방전용량, 충전용량, 효율, 용량유지율)
- 💾 데이터베이스 저장 (자동 테이블 생성)
...
**빠른 시작 (실무용):**
- **unified_analysis.ipynb** ⭐ - 모든 경로 타입을 자동으로 처리
